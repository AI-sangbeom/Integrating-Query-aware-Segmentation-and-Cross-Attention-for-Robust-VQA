...
class Vizwiz(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if temp_name == "COCO":
            ...
            image = "/raid/wonjun/Qwen-VL/segment_contrast/train/" + self.csv_file[idx]['image']
...
#     # Tracking variables 
#     train_accuracy = 0
#     for data, target, dataset_list in train_loader:
...
#     train_loss_val = train_loss / train_iter_size
#     print(train_loss_val)
#     return train_loss_val #, train_acc

# def val(model, val_loader,batch_size):
#     device = torch.device(f"cuda:{dist.get_rank()}")
#     only_check_device = torch.device("cuda:0")
#     print("device", device)
#     val_num_batches = int(math.ceil(len(val_loader.dataset) / int(args.world_size)))
#     val_iter_size = math.ceil(val_num_batches / batch_size)
#     model.eval()
#     # let all processes sync up before starting with a new epoch of training
#     # dist.barrier()
#     val_loss = 0.0
...
#     # Tracking variables 
#     val_accuracy = 0
#     temp_count_valid_check = 0
#     temp_count_total_check = 0
#     with torch.no_grad():
#         for data, target, dataset_list in val_loader:
#             data, target = data.to(device), target.to(device)
#             output = model(data, labels=target)
#             loss = output.loss
#             loss_ = {'loss': torch.tensor(loss.item()).to(device)}
...
#                 if iter_count % print_iter_ == 0:
#                     print(data)
#                     summary_ids = model.module.generate(data.to(device), num_beams=5, max_length=200, early_stopping=True)
#                     predict = [tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=False) for g in summary_ids]
#                     answer = [tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=False) for g in target]
#                     for predict_n in range(len(predict)):
#                         if str(predict[predict_n]) == str(answer[predict_n]):
#                             temp_count_valid_check += 1
#                             temp_count_total_check += 1
#                         else:
#                             temp_count_total_check += 1

#                     print('[Target sample] -- ', answer[0])
#                     print('[Generate sample] -- ', predict[0])
#             iter_count += 1
#             cleanup
#             dist.barrier()
#     print("temp_count_valid_check / temp_count_total_check")
#     print(temp_count_valid_check, " / ", temp_count_total_check)
    
#     val_loss_val = val_loss / val_iter_size
#     print(val_loss_val)
#     return val_loss_val #, val_acc
# torch.nn.parallel에서 DistributedDataParallel을 DDP로 가져옵니다.