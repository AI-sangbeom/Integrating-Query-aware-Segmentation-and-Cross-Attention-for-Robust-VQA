...
tokenizer = AutoTokenizer.from_pretrained("../Qwen_VL_Chat_Seg_sb", trust_remote_code="cpu")
...
seg_config = "CIDAS/clipseg-rd64-refined"
seg_processor = CLIPSegProcessor.from_pretrained(seg_config) 
...
class Vizwiz(Dataset):
    def __init__(self, tokenizer, seg_processor, max_len=400):
        data_path = "/abr/wonjun/cvpr_chal/vizwiz/train.json"
        # data_path = "/raid/wonjun/Qwen-VL/vqav2_gen/train_viz_vqav2_split_seg.json"
        with open(data_path, 'r+') as f:
            self.csv_file = json.load(f)
        self.tokenizer = tokenizer
        self.segment_tokenizer = seg_processor 
        self.max_len = 500
    ...
    def __getitem__(self, idx):
        ...
        if temp_name == "COCO":
            image = "/abr/wonjun/cvpr_chal/vqav2/train2014/" + self.csv_file[idx]['image']
        elif temp_name == "VizWiz":
            image = "/abr/wonjun/cvpr_chal/vizwiz/train/" + self.csv_file[idx]['image']
        ...
        segment_query = self.segment_tokenizer(text=[question], images=[raw_image] * len([question]), padding="max_length",return_tensors="pt") #; print(segment_query)
        # print(segment_query)
        return torch.LongTensor(input_ids[0].tolist()), torch.LongTensor(attention_mask), torch.LongTensor(targets), question, answers, answerable, answer_type, segment_query
...
def get_dataset():
    ...
    train_loaded = Vizwiz(tokenizer, seg_processor)#, transform=transform_train)
...
def run(rank, world_size):
    ...
    seg_config = "CIDAS/clipseg-rd64-refined"
    seg_processor = CLIPSegProcessor.from_pretrained(seg_config) 
    seg_model = CLIPSegForImageSegmentation.from_pretrained(seg_config).eval()
    seg_model.to(device)
    print()
    # model.to(device)
    # model.generation_config = GenerationConfig.from_pretrained(model_model_path, trust_remote_code=True)
    ...
    for epoch in range(args.epochs):
        for phase in ['Train']:
            ...
            if phase == "Train":
                ...
                for input_ids, attention_mask, targets, question, answers, answerable, answer_type, segment_query in train_loader:
                    optimizer.zero_grad()
                    B = segment_query['pixel_values'].size(0)
                    segment_query['pixel_values'] =  segment_query['pixel_values'].squeeze(1)
                    segment_query['input_ids'] = segment_query['input_ids'].squeeze(1)
                    segment_query['attention_mask'] = segment_query['attention_mask'].squeeze(1)
                    seg_feature = seg_model(**segment_query.to(device), output_hidden_states=True)
                    seg_feature = seg_feature.decoder_output.hidden_states[-1]
                    # print("A",seg_feature)
                    seg_feature = torch.Tensor(seg_feature.tolist()).bfloat16().to(device)
                    outputs = model(input_ids=input_ids.to(device), attention_mask=attention_mask.to(device), labels=targets.to(device), seg_image=seg_feature.bfloat16())
                    loss = outputs.loss
                    ...
                    if device == only_check_device:
                        if iter_count % print_iter_ == 0:
                            ...
                            for j in range(len(answer_type)):
                                ...
                                temp_seg_image = seg_feature[j].unsqueeze(0)
                                # print(temp_seg_image)   ; break
                                generated_output = model.module.generate(input_ids=temp_input.to(device),
                                                                         seg_image=temp_seg_image.bfloat16(),
                                                                          # do_sample=True,
                                                                          top_p=0.5,
                                                                          top_k=0,
                                                                          # early_stopping=True,
